...
fun MiAplicacion(ayudante: AyudanteBaseDatos) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        PantallaAgregarUsuario(
            nombre = usuarioSelecionado?.nombre?:" ",
            correo = usuarioSelecionado?.correo?:" ",
            enModoEdicion = enModoEdicion,
            onNombreCambio = { nuevoNombre ->usuarioSelecionado=usuarioSelecionado?.copy(nombre=nuevoNombre) ?:Usuario(0,nuevoNombre,"") },
            onCorreoCambio = { nuevoCorreo ->usuarioSelecionado=usuarioSelecionado?.copy(nombre=nuevoCorreo) ?:Usuario(0,"",nuevoCorreo) },
            onGuardar = {
                if (enModoEdicion) {
                    ...
                    val id = insertarUsuario(ayudante,usuarioSelecionado?.nombre?: "", usuarioSelecionado?.correo?: "").toInt()
                    usuarios.add(Usuario(id,usuarioSelecionado?.nombre?:"", usuarioSelecionado?.correo ?:""))
                }
                enModoEdicion = false
        ...
        PantallaListaUsuarios(
            ...
            onActualizar = { id, nombre, correo ->
                enModoEdicion = true
                usuarioSelecionado = Usuario(id,nombre,correo)
            }
        )
    }
}
            ...
            Row(
                ...
                Row {
                    ...
                    Button(onClick = {
                        ...
                        usuarios.remove(usuario) // Actualiza la lista eliminando el usuario
                    }) {
                        Text("Eliminar")
                    }
                }
